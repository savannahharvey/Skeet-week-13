...
void Skeet::drawLevel() const
{
   ...
   Graphics::drawBackground(dimensions, time.level() * .1, 0.0, 0.0);
   
   // draw the bullseye
   if (bullseye)
      Graphics::drawCrosshair(gun.getAngle(), dimensions);

   // output the gun
   Graphics::drawGun(&gun);
         
   // output the birds, bullets, and fragments
   for (auto& pts : points)
      Graphics::drawPoints(&pts);
   for (auto effect : effects)
      Graphics::drawEffects(effect);
   for (auto bullet : bullets)
      Graphics::drawBullet(bullet);
   for (auto element : birds)
      Graphics::drawBird(element);
   
   // status
   Graphics::drawText(Position(10,                         dimensions.getY() - 30), score.getText()  );
   Graphics::drawText(Position(dimensions.getX() / 2 - 30, dimensions.getY() - 30), time.getText()   );
   Graphics::drawText(Position(dimensions.getX() - 110,    dimensions.getY() - 30), hitRatio.getText());
}
...
void Skeet::drawStatus() const
{
   ...
   if (time.isGameOver())
   {
      ...
      Graphics::drawText(Position(dimensions.getX() / 2 - 30, dimensions.getY() / 2 + 10),
               "Game Over");
   ...
   else
   {
      ...
      Graphics::drawTimer(dimensions, 1.0 - time.percentLeft(),
      ...
      Graphics::drawText(Position(dimensions.getX() / 2 - 110, dimensions.getY() / 2 - 10),
         sout.str());
   }
}
